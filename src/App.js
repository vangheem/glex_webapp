...
class App extends Component {
    ...
    componentDidMount() {
		...
		http(BASE_URL + '@get-token', 'POST', function(xhr){
			self.state.authToken = xhr.responseText;
			http(BASE_URL + '@videos?token=' + self.state.authToken, 'GET', function(xhr){
    ...
    saveVideoEditClicked(e){
		...
		http(BASE_URL + '@edit-data?token=' + self.state.authToken, 'POST', function(xhr){
			self.setState({
				editVideo: null
			});
		}, JSON.stringify({
    ...
    renderEditVideo(){
    	...
    	<div className="modal-content">
            ...
            <form className="col s12">
				<div className="row">
					...
					<div className="col s9">
						<input id="edit-title" placeholder="Title" type="text"
							   value={videoData.Title}
						   	   onChange={self.editVideoField.bind(self, 'Title')}></input>
					</div>
				</div>
				<div className="row">
					<div className="col s3">
					...
					<div className="col s9">
					<input id="edit-plot" placeholder="Plot" type="text"
						   value={videoData.Plot}
						   onChange={self.editVideoField.bind(self, 'Plot')}></input>
				</div>
			</div>
			<div className="row">
				<div className="col s3">
				...
				<div className="col s9">
					<input id="edit-year" placeholder="Year" type="text"
						   value={videoData.Year} onChange={self.editVideoField.bind(self, 'Year')}></input>
				</div>
			</div>
			...
			<div className="row">
				...
				<div className="col s9">
					<input id="edit-rated" placeholder="Rated" type="text"
						   value={videoData.Rated}
					   	   onChange={self.editVideoField.bind(self, 'Rated')}></input>
				</div>
			</div>
			...
			<div className="row">
				...
				<div className="col s9">
					<input id="edit-type" placeholder="Type" type="text"
						   value={videoData.Type}
						   onChange={self.editVideoField.bind(self, 'Type')}></input>
				</div>
			</div>
			...
			<div className="row">
				...
				<div className="col s9">
					<input id="edit-poster" placeholder="Poster" type="text"
						   value={videoData.Poster}
						   onChange={self.editVideoField.bind(self, 'Poster')}></input>
				</div>
			</div>
			...
			<div className="row">
				...
				<div className="col s9">
					<input id="edit-imdb" placeholder="IMDB Rating" type="text"
						   value={videoData.imdbRating}
					   	   onChange={self.editVideoField.bind(self, 'imdbRating')}></input>
				</div>
			</div>
			...
			<div className="row">
				...
				<div className="col s9">
					<input id="edit-runtime" placeholder="Runtime" type="text"
						   value={videoData.Runtime}
						   onChange={self.editVideoField.bind(self, 'Runtime')}></input>
				</div>
			</div>
		</form>
	</div>
    <div className="modal-footer">
		<a className="modal-action modal-close waves-effect waves-red btn-flat "
		   onClick={self.closeVideoEditClicked.bind(self)}>Close</a>
		<a className="modal-action modal-close waves-effect waves-green btn-flat "
		   onClick={self.saveVideoEditClicked.bind(self)}>Save</a>
    </div>
</div>);
    }
    ...
    render() {
		...
		this.getFilteredVideos().forEach(function(videoData){
			...
			var url = BASE_URL + "@download?id=" + video.id + '&token=' + this.state.authToken;
		...
		if(this.state.selectedVideo){
			var url = BASE_URL + "@stream?id=" + this.state.selectedVideo + '&token=' + this.state.authToken;
			selectedVideo = (
		...
		return (
			...
			<div className="input-field col s12">
				<input id="filter" type="text" placeholder="Filter"
				 	   value={this.state.filter} onChange={this.filterChanged.bind(self)}></input>
			</div>
			{videos}
			</div>
		);
